...
class AuthService
{
    public function checkLogin($request)
    {
        try {
            ...
            if (Auth::guard('courier')->attempt($check_data, $remember_me)) {
                session(array('user_name' => Auth::guard('courier')->user()->business_name));
                $response['data'] = true;
    ...
    public function forgetPassword($request)
    {
        try {
            ...
            Mail::send('email.courier.forgot_password', ['token' => $token], function ($message) use ($request) {
    ...
    public function savePassword($request)
    {
        try {
            ...
            if ($updatePassword) {
                Courier::where('email', $updatePassword->email)->update(['password' => Hash::make($request['new_password'])]);
