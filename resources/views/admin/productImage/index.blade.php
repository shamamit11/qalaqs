...
@section('content')
                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="card-header d-flex justify-content-between align-items-center">
                                <h3>Product Images Management</h3>
                                <nav class="navbar navbar-light">
                                    ...
                                    <a href="{{ route('admin-product-image-add') }}" class="btn btn-primary my-2 my-sm-0 ms-1">
                                        Add</a>
                                </nav>
                            </div>
                            <div class="card-body">
                                @if ($product_images->count() > 0)
                                    <table class="table">
                                        ...
                                        <tbody>
                                            @foreach ($product_images as $product_image)
                                                <tr id="tr{{ $product_image->id }}">
                                                    <td>{{ $count++ }}</td>
                                                    <td>{{ $product_image->category->name }} / {{ $product_image->subcategory->name }} / {{ $product_image->make->name }} / {{ $product_image->model->name }} / {{ $product_image->year->name }}</td>
                                                    <td><label class="switch" style="margin: 0 auto">
                                                            <input class="switch-input switch-status" type="checkbox"
                                                                data-id="{{ $product_image->id }}"
                                                                data-status-value="{{ $product_image->status }}"
                                                                @if ($product_image->status == 1) checked @endif /> <span
                                                                class="switch-label" data-on="Show" data-off="Hide"></span>
                                                            <span class="switch-handle"></span> </label></td>
                                                    <td style="text-align:center"><a
                                                            href="{{ route('admin-product-image-add', ['id=' . $product_image->id]) }}"
                                                        ...
                                                        @if (checkIfUserIsSuperAdmin())
                                                            <button type="button"
                                                                ...
                                                                data-id="{{ $product_image->id }}"><span class="icon"><i
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-md-8 col-sm-6 col-xs-12">
                                            <div class="float-end"> {{ $product_images->links('pagination::bootstrap-4') }}
                                            </div>
                                        </div>
                                    </div>
                                @else
    ...
    @section('footer-scripts')
        @include('admin.productImage.js.index')
    @endsection
